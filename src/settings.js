...
let settingsJson;
try {
  settingsJson = fs.readFileSync("settings/settings.json", { encoding: "utf8" });
} catch(err) {
  if(err.code === 'ENOENT') {
    settingsJson = fs.readFileSync("settings.json", { encoding: "utf8" });
    console.warn("Loading settings.json from the root directory is deprecated and may stop working in a future release.");

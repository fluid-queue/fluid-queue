...
const defaultActivated = ['smm2','customcode', 'customlevel', 'smm2-regex', 'smm1'];

const fileEnding = '.js';
const loadFiles = (path) => {
    const result = {};
    const files = fs.readdirSync(path).filter(file => file.endsWith(fileEnding));
...
const resolvers = {
    ...
    get resolvers() {
        return this.activated.filter(name => name in this.available).map(name => this.available[name]).filter(resolver => resolver != null);
    },
    /**
    ...
    resolveArguments(string) {
        const resolve = (resolver, string, types) => {
            if (resolver != null) {
                if (resolver.resolveArguments != null && typeof resolver.resolveArguments === 'function') {
        ...
        if (resolverName in this.available && this.activated.includes(resolverName)) {

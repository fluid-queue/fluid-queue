...
function courseIdValidity(checksum, courseIdString) {
...
const extractValidCode = (levelCode) => {
  ...
  if (match) {
    let checksum = `${match[1]}`;
    let courseIdString = `${match[2]}${match[3]}`;
    let validity = courseIdValidity(checksum, courseIdString);
    return {
      valid: validity,

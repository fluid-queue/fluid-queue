...
test("online users", async () => {
  ...
  await expect(twitch.getOnlineUsers(settings.channel)).resolves.toEqual(
    new Set([])
  );

  // change chatters mock and compare with result
  setChatters({
    broadcaster: ["liquidnya"],
    vips: ["redzebra_"],
    moderators: ["helperblock"],
    staff: [],
    admins: [],
    global_mods: [],
    viewers: [],
  ...
  await expect(twitch.getOnlineUsers(settings.channel)).resolves.toEqual(
    new Set(["liquidnya", "helperblock", "redzebra_"])
  );
  ...
  await expect(twitch.getOnlineUsers(settings.channel)).resolves.toEqual(
    new Set(["liquidnya", "helperblock", "redzebra_", "furretwalkbot"])
  );
